...
import { Coins, Wallet, Gift, ShoppingBag, CheckCircle2, ArrowRight } from 'lucide-react';
          ...
          <div className="relative z-10 grid md:grid-cols-2 gap-12 items-center">
            <div>
              ...
              <ul className="space-y-4 mb-8">
                <li className="flex items-start gap-3">
                  ...
                  <div><span className="font-bold block text-lg">Free Membership</span></div>
                </li>
                <li className="flex items-start gap-3">
                  <CheckCircle2 className="text-yellow-400 flex-shrink-0" />
                  <div><span className="font-bold block text-lg">Simple Math: ₹100 = 1 Coin</span></div>
                </li>
              </ul>
              <Button 
                variant="primary" 
                className="bg-yellow-500 hover:bg-yellow-600 text-gray-900 border-none group"
                href={getWhatsAppLink("Hello! I want to start the Sweet Coins Loyalty Program for my customers. How can I enable this in my bakery system?")}
                target="_blank"
              >
                Launch Loyalty Rewards <ArrowRight className="ml-2 group-hover:translate-x-1 transition-transform" />
              </Button>
            </div>
            <div className="bg-white/10 p-8 rounded-2xl backdrop-blur-sm border border-white/20">
               ...
               <div className="bg-yellow-500 text-gray-900 font-bold text-center py-4 rounded-xl shadow-lg transform scale-105">
                 Get 1 Sweet Coin for every ₹100 spent
               </div>
            </div>
          </div>
        </div>
