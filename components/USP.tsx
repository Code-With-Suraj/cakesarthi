...
import { CalendarHeart, Gift, IndianRupee, LayoutDashboard, PenTool, Sparkles, ArrowRight } from 'lucide-react';
...
const usps = [
  ...
  },
  {
    ...
    subtitle: "Baaki Paisa System Le Aayega",
  ...
  },
  {
    ...
    description: "Orders dekho. Revenue track karo. Aaj kiska birthday hai dekho. Pending payment check karo. Ek simple dashboard. Zero technical tension.",
  ...
  },
  {
    ...
    description: "Customer apna design, budget aur details form me submit karega. Sab details clean format me aapko milengi. No confusion. No missing info.",
    icon: PenTool,
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
          {usps.map((usp, idx) => (
            <div 
              key={idx} 
              className={`group relative p-8 rounded-3xl border-2 transition-all duration-300 hover:shadow-2xl hover:-translate-y-2 bg-white ${usp.color}`}
            >
              ...
              <h4 className="text-base font-bold text-gray-700 mb-4 opacity-80">"{usp.subtitle}"</h4>
              <p className="text-gray-600 leading-relaxed mb-6">{usp.description}</p>
              <div className="inline-block px-3 py-1 rounded-md bg-white border border-gray-200 text-xs font-bold uppercase tracking-wider text-gray-800 shadow-sm">{usp.highlight}</div>
            </div>
          ))}
        </div>
        ...
        <div className="mt-16 text-center">
          <Button 
            size="lg" 
            href={getWhatsAppLink("Hello! I want to upgrade my bakery to a Digital System. Please tell me about these 5 game-changing features.")}
            target="_blank"
            className="group"
          >
            I want this for my Bakery <ArrowRight className="ml-2 group-hover:translate-x-1 transition-transform" />
          </Button>
        </div>
      </div>
    </section>
